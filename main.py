...
CORS_ORIGINS = [
    ...
    f"http://localhost:8037",
    f"http://localhost:8038",
    f"http://127.0.0.1:8036",
    f"http://127.0.0.1:8037",
...
- When you lack sufficient information to answer a question, use web search to find the latest and most relevant information
...
- Acknowledge limitations honestly if you cannot find a reliable answer

Workflow:
1. Analyze the user's question to understand what is being asked
2. If the answer is within your existing knowledge and likely still current, respond directly
3. If the question requires up-to-date information or verification, perform a web search
4. If the question is ambiguous, ask the user a clarifying question
5. Synthesize information and deliver a clear, professional response

Tone: Professional, direct, and helpful. Avoid unnecessary filler or overly casual language.""",
...
async def get_frontend_info():
    ...
    return {
        "backend_url": f"http://localhost:8035",
        "frontend_url": f"http://localhost:8036",
        "cors_enabled": True,
...
if __name__ == "__main__":
    ...
    print(f"[BACKEND] Backend URL: http://localhost:8035")
    print(f"[FRONTEND] Frontend URL: http://localhost:8036")
    print(f"[CORS] CORS: Enabled for frontend connections")
    print(f"[DOCS] API Docs: http://localhost:8035/docs")
    print(f"[HEALTH] Health: http://localhost:8035/health")
